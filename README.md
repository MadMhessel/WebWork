...
При большом количестве источников используйте новые флаги командной строки:

* `--max-channels-per-iter` — ограничивает число каналов в одной итерации
  цикла. Прогресс сохраняется в `var/state.json`, поэтому следующий запуск
  продолжит обход со следующего чанка.
* `--fetch-workers` — количество асинхронных воркеров Telethon. Каждый воркер
  оборачивается в семафор и общий токен-бакет, поэтому даже при высоком
  параллелизме мы не выходим за лимиты API.
* `--messages-per-channel` — сколько сообщений забирать из каждого канала.
* `--rate` — глобальный лимит операций Telegram (запросов в секунду). Это верхний
  предел токен-бакета, который используется как при чтении через Telethon, так
  и при публикации через Bot API.
* `--flood-sleep-threshold` — передаётся в Telethon, чтобы библиотека сама
  пережидала короткие `FloodWait`, а длинные задержки обрабатываются вручную.

Перед публикацией каждое обращение к Bot API проходит через два токен-бакета:
глобальный (≈30 операций/с по умолчанию) и пер-чатный (1 сообщение/с для
личных чатов и до 20 сообщений/минуту для каналов/групп). Это помогает избежать
ошибок 429. Для массовых рассылок лимит можно официально поднять через опцию
**paid broadcasts** в Bot API.

Вы можете изменить значения по умолчанию через `.env` (`TELEGRAM_MAX_CHANNELS_PER_ITER`,
`TELEGRAM_FETCH_WORKERS`, `TELEGRAM_MESSAGES_PER_CHANNEL`, `TELEGRAM_RATE`,
